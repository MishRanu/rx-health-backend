...
$suggest = array();
	try 
		{	
			...
			$result = $db->prepare("SELECT DID,PFID from Booking where AID = :AID");
			$result->bindParam(':AID', $obj['AID'], PDO::PARAM_STR);
			$result->execute();
			$row = $result->fetch();

			$result2 = $db->prepare("SELECT * from doctorfinalsymptom where PFID = ANY(SELECT a.PFID FROM doctormedicine dm inner join Booking a on dm.AID=a.AID where DID=:DID) and PFID!=:PFID");
			...
			{ 
				...
				if($match_percent>20)
				{
					$result19 = $db->prepare("SELECT AID from Booking where PFID=:PFID");
					$result19->bindParam(':PFID', $mainsym[$n][0]['PFID'], PDO::PARAM_STR);
					$result19->execute();
					$row19 = $result19->fetch();

					$match_percent = sprintf('%0.0f', $match_percent);
					$result20 = $db->prepare("SELECT m.Medicine, dtm.Dosage, dtm.Type, dtm.Morning, dtm.Afternoon, dtm.Night, dtm.IsAfter, dtm.OnNeed, dtm.Days, dtm.MedDate
