	...
	try 
		{
			...
			$pfid = $query->fetch();
				...
				$result = $db->prepare("SELECT m.Medicine, dtm.Dosage, dtm.Type, dtm.Morning, dtm.Afternoon, dtm.Night, dtm.IsAfter, dtm.OnNeed, dtm.Days, dtm.MedDate
			...
			else
			{
				if(!is_null($row13['Notes']))
					$response['Notes'] = (string)$row13['Notes'];
				else
					$response['Notes'] = "";
			}
