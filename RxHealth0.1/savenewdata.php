	...
	try
		{
			...
			$query2->execute();
      $query3 = $db->prepare("SELECT CommuID from Dconnection where UserID=:UserID and UserType=3");
			...
			if($count2 == 0){
				...
				$result0 = $db->prepare("INSERT INTO user (FName, LName, Phone, Password, IsDoctor, IsRegistered, IsLoggedin, IsUpdated, IsSelfCreated, RegDate) VALUES (:FName, '', :Phone, :Password, 0, 1, 0, 0, 0, Now())");
				$result0->bindParam(':FName', $obj['Name'], PDO::PARAM_STR);
				$result0->bindParam(':Phone', $obj['Phone'], PDO::PARAM_STR);
				$result0->bindParam(':Password', $password, PDO::PARAM_STR);
				$result0->execute();
				...
				$query0->execute();
				$row =  $query0->fetch();
                                $result10 = $db->prepare("INSERT INTO Dconnection (CommuID, UserID, UserType) VALUES (1, :UserID, 1)");
      ...
      if($count4 == 0){
			...
			$result3 = $db->prepare("INSERT INTO appointment3 (DID, PID, PFID,Status) VALUES (:DID, :PID, :PFID,'Active')");
			...
			if($count2==0)
			{
				$result4 = $db->prepare("INSERT INTO patientprofile (PID, CPFID) VALUES (:PID, :CPFID)");
				$result4->bindParam(':PID', $row['UserID'], PDO::PARAM_STR);
				$result4->bindParam(':CPFID', $pfid, PDO::PARAM_STR);
				$result4->execute();
			}
			else
			{
