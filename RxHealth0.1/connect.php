...
try{
  if(!$obj['CommuID']){
    $query = $db->prepare("SELECT cd.CommuID FROM ComDetails cd INNER JOIN Dconnection dc ON dc.CommuID = cd.CommuID WHERE dc.UserID = :UserID AND UserType = 3 AND cd.ComType = 0");
