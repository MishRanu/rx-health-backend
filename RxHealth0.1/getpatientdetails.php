...
$data = array();
	...
	try 
		{
			$result = $db->prepare("SELECT AID, DID, PID, PFID from appointment3 where DID=:DID and PID=:PID and Status='Complete' ");
			...
			while($row = $result->fetch())
			{
				...
				$result5 = $db->prepare("SELECT m.Medicine, dtm.Dosage, dtm.Type, dtm.Morning, dtm.Afternoon, dtm.Night, dtm.IsAfter, dtm.OnNeed, dtm.Days, dtm.MedDate
				...
				while ($row5 = $result5->fetch()) 
				{
					...
					$meddate = date("d/m/y", strtotime($row5['MedDate']));
				}
