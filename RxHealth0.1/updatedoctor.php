	...
	try 
		{
			...
			$result = $db->prepare("UPDATE doctorprofile SET ExStart=:ExStart, Summary = :Summary, RegNo = :RegNo, RegAssoc = :RegAssoc, RegYear= :RegYear where DID=:UserID");
			...
			// $result3 = $db->prepare("DELETE from doctorsubspec where DID=:UserID");
			// $result3->bindParam(':UserID', $obj['UserID'], PDO::PARAM_STR);
			// $result3->execute();
			// $result4 = $db->prepare("INSERT INTO doctorsubspec (DID,DocSubSpec) VALUES (:UserID, :DocSubSpec)");
			// $result4->bindParam(':DocSubSpec', $obj['SubSpeciality'], PDO::PARAM_STR);
